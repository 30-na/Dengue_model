...
load("processedData/R0MaxStatMapNoPrecip.rda")
...
plot_map = function(variables, title){
    ...
    g = ggplot(
        ...
        facet_wrap(
            . ~ variable,
            ncol = 1,
            labeller = as_labeller(c(mean_ber_disc = "(A) Mean annual R0 calculated over the period 1950–2020 (No Precipitation)",
                                     max_ber_disc = "(B) R0 peak that represents the largest monthly value over period 1950–2020 (No Precipitation)"))) +
        ylim(c(-62, 90)) +
    ...
    ggsave(paste0("Figures/fig1NoPrecip.jpg"),
           g,
